...
const ordersApi = {
  getReady: async (page: number = 1, limit: number = 10) => {
    const response = await axiosInstance.get(`/api/restaurants/orders/ready?page=${page}&limit=${limit}`);
...
const ReadyOrdersPage = () => {
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      setApiLoading(true);
      const response = await ordersApi.getReady(page, limit);
      if (response.success) {
  ...
  const confirmStatusUpdate = async () => {
    ...
    try {
      let response;
      if (statusConfirm.newStatus === 'served') {
        response = await ordersApi.updateToServed(statusConfirm.orderId);
      } else if (statusConfirm.newStatus === 'completed') {
        response = await ordersApi.updateToCompleted(statusConfirm.orderId);
      }
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
        {orders.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No ready orders found</h3>
            <p className="text-gray-500 dark:text-gray-400">No ready orders available at the moment.</p>
          </div>
        ) : (
                ...
                {orders.map((order, index) => (
                  <TableRow key={order._id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${index % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50/50 dark:bg-gray-800/20'}`}>
                    ...
                    <TableCell className="px-6 py-4 whitespace-nowrap text-center">
                      ...
                      >
                        {getNextStatuses(order.status, order.orderType).map(status => (
