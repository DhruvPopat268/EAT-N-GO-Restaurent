...
const ordersApi = {
  getReady: async (page: number = 1, limit: number = 10, filters?: any) => {
    ...
    const response = await axiosInstance.get(`/api/restaurants/orders/ready?${params.toString()}`);
...
const ReadyOrdersPage = () => {
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      ...
      const response = await ordersApi.getReady(page, limit, filters);
      if (response.success) {
