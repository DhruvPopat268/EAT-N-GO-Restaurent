...
const ordersApi = {
  getCancelled: async (page: number = 1, limit: number = 10) => {
    const response = await axiosInstance.get(`/api/restaurants/orders/cancelled?page=${page}&limit=${limit}`);
...
const CancelledOrdersPage = () => {
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      setApiLoading(true);
      const response = await ordersApi.getCancelled(page, limit);
      if (response.success) {
