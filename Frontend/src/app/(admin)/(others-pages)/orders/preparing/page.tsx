...
const ordersApi = {
  getPreparing: async (page: number = 1, limit: number = 10) => {
    const response = await axiosInstance.get(`/api/restaurants/orders/preparing?page=${page}&limit=${limit}`);
...
const PreparingOrdersPage = () => {
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      setApiLoading(true);
      const response = await ordersApi.getPreparing(page, limit);
      if (response.success) {
