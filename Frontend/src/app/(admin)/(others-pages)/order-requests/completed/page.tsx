...
export default function CompletedOrderRequests() {
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/api/restaurants/order-requests/completed?page=${page}&limit=${limit}`);
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Completed Order Requests</h1>
        <p className="text-gray-600 dark:text-gray-400">Order requests that have been completed</p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        {orders.length === 0 && (
          <div className="text-center py-8">
            <p className="text-gray-500 dark:text-gray-400">No completed order requests found</p>
          </div>
        )}
      </div>
