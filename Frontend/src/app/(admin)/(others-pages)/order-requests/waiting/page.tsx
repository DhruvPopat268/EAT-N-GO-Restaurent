...
export default function WaitingOrderRequests() {
  ...
  useOrderNotifications("Waiting Order Requests");
  useOrderRequestNotifications("Waiting Order Requests");

  // Direct socket listener for notifications
  useEffect(() => {
    if (!socket || !isConnected) {
      console.log('📱 Waiting Orders page: Socket not ready', { socket: !!socket, isConnected });
      return;
    }

    const handleNewOrder = (orderData: any) => {
      const timestamp = new Date().toLocaleString();
      console.log(`🔔 [${timestamp}] WAITING ORDERS PAGE - New order received:`, {
      ...
      if (orderData.status === 'waiting' || orderData.status === 'confirmed') {
        ...
        // Add to waiting orders list if on page 1 and status is waiting
        if (pagination.page === 1 && orderData.status === 'waiting') {
          console.log(`➕ [${timestamp}] Adding to waiting orders list`);
    ...
    console.log('📱 Waiting Orders page: Registering socket listener');
    socket.on('new-order', handleNewOrder);

    return () => {
      console.log('📱 Waiting Orders page: Removing socket listener');
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      ...
      toast.error('Failed to fetch waiting orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePageChange = (page: number) => {
    ...
    try {
      const payload = { orderReqId: orderId };
      await axiosInstance.patch(`/api/restaurants/order-requests/${action}`, payload);
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Waiting Order Requests</h1>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        {orders.length === 0 && (
          <div className="text-center py-8">
            <p className="text-gray-500 dark:text-gray-400">No waiting orders found</p>
          </div>
        )}
      </div>
