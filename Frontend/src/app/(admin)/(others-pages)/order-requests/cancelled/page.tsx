...
export default function CancelledOrderRequests() {
  ...
  useOrderRequestNotifications("Cancelled Order Requests");

  // Direct socket listener for notifications
  useEffect(() => {
    if (!socket || !isConnected) {
      console.log('📱 Cancelled Orders page: Socket not ready', { socket: !!socket, isConnected });
      return;
    }

    const handleNewOrder = (orderData: any) => {
      const timestamp = new Date().toLocaleString();
      console.log(`🔔 [${timestamp}] CANCELLED ORDERS PAGE - New order received:`, {
    ...
    console.log('📱 Cancelled Orders page: Registering socket listener');
    socket.on('new-order', handleNewOrder);

    return () => {
      console.log('📱 Cancelled Orders page: Removing socket listener');
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      ...
      const response = await axiosInstance.get(`/api/restaurants/order-requests/cancelled?${params.toString()}`);
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Cancelled Order Requests</h1>
        <p className="text-gray-600 dark:text-gray-400">Orders that have been cancelled by users</p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        {orders.length === 0 && (
          <div className="text-center py-8">
            <p className="text-gray-500 dark:text-gray-400">No cancelled orders found</p>
          </div>
        )}
      </div>
