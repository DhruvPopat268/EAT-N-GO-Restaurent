...
export default function RejectedOrderRequests() {
  ...
  const fetchOrders = async (page: number = pagination.page, limit: number = pagination.limit) => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/api/restaurants/order-requests/rejected?page=${page}&limit=${limit}`);
      setOrders(response.data.data);
      setPagination(response.data.pagination);
    } catch (error) {
      toast.error('Failed to fetch rejected orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Rejected Order Requests</h1>
        <p className="text-gray-600 dark:text-gray-400">Orders that have been rejected</p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        {orders.length === 0 && (
          <div className="text-center py-8">
            <p className="text-gray-500 dark:text-gray-400">No rejected orders found</p>
          </div>
        )}
      </div>
