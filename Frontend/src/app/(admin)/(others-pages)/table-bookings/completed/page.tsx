...
const CompletedTableBookings = () => {
  ...
  useTableBookingSocket({
    pageName: "Completed Table Bookings"
  });

  const fetchCompletedBookings = async (page: number = pagination.currentPage, limit: number = pagination.limit, applyFilters: boolean = false) => {
    try {
      setLoading(true);
      ...
      const { data } = await axiosInstance.get('/api/restaurants/table-bookings/completed', {
        params
      });
      ...
      console.error('Error fetching completed table bookings:', error);
      toast.error('Error fetching completed table bookings');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    fetchCompletedBookings(1, pagination.limit, !!hasActiveFilters);
  ...
  const handlePageChange = (page: number) => {
    const hasActiveFilters = filters.search || filters.slot || filters.startDate || filters.endDate;
    fetchCompletedBookings(page, pagination.limit, hasActiveFilters);
  };

  const handleLimitChange = (limit: number) => {
    const hasActiveFilters = filters.search || filters.slot || filters.startDate || filters.endDate;
    fetchCompletedBookings(1, limit, hasActiveFilters);
  };
  ...
  const applyFiltersWithValues = (filterValues = filters) => {
    ...
    setLoading(true);
    axiosInstance.get('/api/restaurants/table-bookings/completed', { params })
      .then(({ data }) => {
        if (data.success) {
          setBookings(data.data.bookings);
          setPagination(data.data.pagination);
  ...
  const clearFilters = () => {
    ...
    fetchCompletedBookings(1, pagination.limit, false);
  };
  ...
  if (loading) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Completed Table Bookings</h1>
          <p className="text-gray-600 dark:text-gray-400">View completed table bookings</p>
        </div>
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Showing {bookings.length} of {pagination.totalCount} completed bookings
          {hasActiveFilters && (
                  ...
                  <td colSpan={12} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                        No Completed Table Bookings Found
                      </h3>
                      <p className="text-gray-500 dark:text-gray-400">
                        There are currently no completed table bookings to display.
                      </p>
                    </div>
                  </td>
                </tr>
...
export default CompletedTableBookings;