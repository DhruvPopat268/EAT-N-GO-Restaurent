...
const ArrivedTableBookings = () => {
  ...
  useTableBookingSocket({
    pageName: "Arrived Table Bookings"
  });

  const fetchArrivedBookings = async (page: number = pagination.currentPage, limit: number = pagination.limit, applyFilters: boolean = false) => {
    try {
      setLoading(true);
      ...
      const { data } = await axiosInstance.get('/api/restaurants/table-bookings/arrived', {
        params
      });
      ...
      console.error('Error fetching arrived table bookings:', error);
      toast.error('Error fetching arrived table bookings');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePageChange = (page: number) => {
    const hasActiveFilters = filters.search || filters.slot || filters.startDate || filters.endDate;
    fetchArrivedBookings(page, pagination.limit, hasActiveFilters);
  };

  const handleLimitChange = (limit: number) => {
    const hasActiveFilters = filters.search || filters.slot || filters.startDate || filters.endDate;
    fetchArrivedBookings(1, limit, hasActiveFilters);
  };
  ...
  const applyFiltersWithValues = (filterValues = filters) => {
    ...
    setLoading(true);
    axiosInstance.get('/api/restaurants/table-bookings/arrived', { params })
      .then(({ data }) => {
        if (data.success) {
          setBookings(data.data.bookings);
          setPagination(data.data.pagination);
  ...
  const clearFilters = () => {
    ...
    fetchArrivedBookings(1, pagination.limit, false);
  };
  ...
  const confirmStatusUpdate = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        fetchArrivedBookings(); // Refresh the bookings list
      } else {
  ...
  if (loading) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Arrived Table Bookings</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage bookings where customers have arrived</p>
        </div>
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Showing {bookings.length} of {pagination.totalCount} arrived bookings
          {hasActiveFilters && (
                  ...
                  <td colSpan={11} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                        No Arrived Table Bookings Found
                      </h3>
                      <p className="text-gray-500 dark:text-gray-400">
                        There are currently no bookings where customers have arrived.
                      </p>
                    </div>
                  </td>
                </tr>
...
export default ArrivedTableBookings;